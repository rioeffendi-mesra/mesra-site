...
A **point** CEMS measures at a single spot, which must be **at least 1.0 m from the duct wall** or sit over the centroidal area (§4.2.6). A **path** CEMS shoots a beam across the duct — and remember from Part 3 that DOE accepts **double-pass only**; the path must satisfy one of three siting options (§4.2.7): lie entirely within 1 m of the wall, have **at least 70% of it inside the inner 50%** of the cross-section, or sit centrally over the centroidal area. The **manual SRM**, meanwhile, doesn't measure at one place at all: it traverses a grid of points set at the centres of equal areas, and ISO 9096 scales that grid with duct size — a single centred point for the smallest ducts, up to seventeen for one over 2 m across.
