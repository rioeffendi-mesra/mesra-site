...
description: "Why Malaysia's Clean Air Regulations 2014 judge most palm oil mill biomass boilers on a 2.5 kg/hour dust-load threshold, not a capacity cutoff."
...
<p class="fig-title">Monitoring frequency by dust load · Second Schedule, Part I, para 2</p>
<div class="rep-rows">
<div class="rep-row"><span class="rep-k">a</span><span class="rep-body"><span class="rep-name">&gt;0.44 – &lt;1.0 kg/h</span><span class="rep-desc">Periodic stack testing, once a year.</span></span></div>
<div class="rep-row"><span class="rep-k">b</span><span class="rep-body"><span class="rep-name">≥1.0 – &lt;1.5 kg/h</span><span class="rep-desc">Periodic stack testing, twice a year.</span></span></div>
<div class="rep-row"><span class="rep-k">c</span><span class="rep-body"><span class="rep-name">≥1.5 – &lt;2.0 kg/h</span><span class="rep-desc">Periodic stack testing, three times a year.</span></span></div>
<div class="rep-row"><span class="rep-k">d</span><span class="rep-body"><span class="rep-name">≥2.0 – &lt;2.5 kg/h</span><span class="rep-desc">Periodic stack testing, four times a year.</span></span></div>
<div class="rep-row hl"><span class="rep-k">e</span><span class="rep-body"><span class="rep-name">≥2.5 kg/h</span><span class="rep-desc">Continuous — a CEMS is required (30-minute averaging).</span></span></div>
</div>
<figcaption>TPM limit for solid fuel is 150 mg/m³ at 12% CO₂. Carbon monoxide sits separately at 1,000 mg/m³ and stays periodic regardless of dust load — more on that later in this series.</figcaption>
</figure>
...
Put together, Reg 12(3)'s transmissometer requirement and row (e) above are why the DOE CEMS Guidelines (Table 2.1) list **both** opacity (20%) and Total PM (150 mg/m³ where dust load is ≥2.5 kg/h) as the two parameters a "Fuel Burning Equipment" CEMS must cover once you cross the threshold — the opacity monitor and the continuous dust monitoring converge at the same 2.5 kg/hour line, rather than triggering two independently timed obligations.
...
Neither CAR 2014 nor the DOE CEMS Guidelines spell out a desk formula for estimating "potential to emit" before you've ever tested. In practice, the number that puts you in row (a) through (e) above comes from a stack test carried out to Malaysia's reference method for particulate. **Regulation 23 of CAR 2014** requires sampling and analysis of emissions to be carried out to **Malaysian Standard MS 1596** (or MS 1723, a US EPA method, or any other standard determined by the Director General) — and MS 1596 is Malaysia's own adoption of the international gravimetric method, **ISO 9096**. The test measures concentration and stack gas flow together, producing the **mass emission rate** — your dust-load figure in kg/h. We cover how to read that report end to end in [reading a stack-test report]({{ '/insights/reading-a-stack-test-report/' | relative_url }}), and how MS 1596, ISO 9096 and their European sibling EN 13284 relate to each other in [MS 1596, ISO 9096 and EN 13284: how Malaysia's stack-test standard fits the international picture]({{ '/insights/ms-1596-iso-9096-en-13284-particulate-standards/' | relative_url }}). There's no separate calculation to do — the test you're already required to run periodically is what tells you which row of the table you're in, and whether you're getting close to the next one.

One thing worth flagging if your mill sits anywhere near the threshold: **MS 1596 is currently under revision.** The edition in force, MS 1596:2003 (confirmed 2013), is built on the 1992 edition of ISO 9096, and its own text acknowledges that accuracy degrades — inaccuracy exceeding ±10% — below 50 mg/m³. That's well under your 150 mg/m³ TPM limit, so it isn't the range that decides *your* number, but it's the reason the standard is being realigned to the current **ISO 9096:2017** and the European low-dust method **EN 13284-1**. We explain why that revision is happening and where it's heading in [The MS 1596 revision: why Malaysia's dust standard is being rewritten]({{ '/insights/ms-1596-revision-low-dust-alignment/' | relative_url }}).

## What this means for your mill

Two practical positions follow from all this:

- **If your last stack test's mass emission rate is below 2.5 kg/hour**, you're compliant on periodic testing — at whichever of the four frequencies matches your number. We walk through what that actually looks like operationally, and what tends to move that number year to year, in [Part 2 of this series]({{ '/insights/palm-oil-mill-periodic-testing-tiers-dust-load/' | relative_url }}).
- **If it's at or above 2.5 kg/hour**, DOE's expectation shifts from periodic testing to continuous monitoring — a CEMS. From there, the requirements are the same ones any regulated industry faces: registration with DOE, a QAL1-certified system through a DOE-registered consultant, correct siting, and QAL2 verification before the data counts. Our [Buying a CEMS series]({{ '/insights/need-a-cems-clean-air-regulations-2014/' | relative_url }}) walks through that process from the beginning, and once you're evaluating quotations, [reading a QAL1 certificate]({{ '/insights/qal1-certificate-tpm-opacity-biomass-boiler/' | relative_url }}) is worth doing before you sign anything — your mill's 150 mg/m³ TPM limit needs to sit inside the certified range, not just the sales brochure. One thing worth planning for early: because biomass fuel varies batch to batch, mills running a CEMS may need a [fresh QAL2 every year rather than the usual annual surveillance test]({{ '/insights/changes-that-trigger-new-qal2-cems/' | relative_url }}) — worth budgeting for once you're on continuous monitoring.
...
**Not sure where your mill's last test puts you, or overdue for one?** [Talk to us]({{ '/' | relative_url }}#contact) — we help palm oil mill operators read their numbers and plan ahead of the threshold, drawing on the DOE CEMS Guidelines we helped write.
